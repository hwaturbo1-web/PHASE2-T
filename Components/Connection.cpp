...
void Connection::Draw(Output* pOut)
{
    ...
    if (label != "")
    {
        ...
        // Draw label slightly offset from the line
...
{
    // Get current positions from pins (they're the source of truth)
    int x1, y1, x2, y2;
    ...
    {
        // Shouldn't happen, but use stored coords as fallback
        x1 = m_GfxInfo.x1;
        y1 = m_GfxInfo.y1;
        x2 = m_GfxInfo.x2;
        y2 = m_GfxInfo.y2;
    ...
    // Calculate the middle X for the vertical segment
    int midX = (x1 + x2) / 2;

    // Use larger tolerance to make clicking easier
    const int TOLERANCE = 25;

    // Segment 1: Horizontal line from source (x1, y1) to middle (midX, y1)
    ...
    // Segment 2: Vertical line from (midX, y1) to (midX, y2)
    ...
    // Segment 3: Horizontal line from middle (midX, y2) to destination (x2, y2)
